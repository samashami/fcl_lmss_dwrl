...
def _build_action(strategy_id: int) -> Dict[str, Any]:
    s = STRATEGY_PALETTE.get(int(strategy_id), STRATEGY_PALETTE[1])
    return {
        "lr_req": float(s["lr"]),
...
def lmss_decide_action_local_dwrl(
    ...
    # Deterministic rules first (fast + robust fallback)
    g = state.get("global", {})
    ...
    if div > 0.10:
        return _build_action(4)
    if forget_mean > 0.05:
        return _build_action(2)
    if dval > 0.01 and div < 0.05:
...
Return only:
{{"strategy_id": <int>, "reasoning": "<short sentence>"}}
"""
