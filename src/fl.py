...
class Server:
    ...
    def average(self, models: List[nn.Module]) -> nn.Module:
        ...
        - For non-float tensors (e.g., BatchNorm num_batches_tracked), just copy from model[0].
        """
        avg = deepcopy(models[0]).to(self.device)
        with torch.no_grad():
            keys = avg.state_dict().keys()
            for k in keys:
                t0 = models[0].state_dict()[k]
                if torch.is_floating_point(t0):
                    s = sum(m.state_dict()[k].to(self.device) for m in models) / float(len(models))
                    avg.state_dict()[k].copy_(s)
                else:
                    # ints/bools/etc: keep from first model
                    avg.state_dict()[k].copy_(t0.to(self.device))
...
class Client:
    ...
    def train_one_epoch(
        ...
        for b, (x_new, y_new) in enumerate(self.loader, 1):
            # keep a copy of the "new stream" batch for replay buffer
            x_new = x_new.to(self.device, non_blocking=True)
            y_new = y_new.to(self.device, non_blocking=True)
            ...
            if self.replay is not None and replay_ratio > 0.0:
                rx, ry = self.replay.sample_like(x_new.size(0), device=self.device, ratio=replay_ratio)
                if rx is not None:
                    rx = rx.to(self.device, non_blocking=True)
                    ry = ry.to(self.device, non_blocking=True)
                    x = torch.cat([x_new, rx], dim=0)
                    y = torch.cat([y_new, ry], dim=0)
            ...
            # ✅ FIX #2: store ONLY new batch (not replay-mixed)
            if self.replay is not None:
                self.replay.add_batch(x_new.detach().cpu(), y_new.detach().cpu())
